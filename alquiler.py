...
class Alquiler():
    ...
    def altaContrato(self):
        ...
        try:
            ...
            mbox.setStandardButtons(QtWidgets.QMessageBox.StandardButton.Ok)
            mbox.exec()
        Alquiler.cargaTablaContratos(self)
        Propiedades.cargaTablaPropiedades(self,0)

    def cargaTablaContratos(self, idContrato=None):
        try:
            if idContrato is None:
                listado = conexion.Conexion.listadoVentas(self)
            else:
                listado = conexion.Conexion.listadoVentas(self, idContrato)
            ...
            for registro in listado:
                ...
                Ventas.botonesdel[-1].clicked.connect(lambda checked: Ventas.eliminar_contrato(self, str(registro[0])))
